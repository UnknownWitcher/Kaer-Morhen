...
if [[ "${MOUNT_NAME}" == "series" ]]; then
    RCLONE_API_PORT="5579"
    RCLONE_MOUNT="series:"
    RCLONE_MOUNT_PATH="/mnt/shared/fusemounts/series"
elif [[ "${MOUNT_NAME}" == "movies" ]]; then
    RCLONE_API_PORT="5578"
    RCLONE_MOUNT="movies:"
    RCLONE_MOUNT_PATH="/mnt/shared/fusemounts/movies"
else
    echo "'${MOUNT_NAME}' does not exist" | tee -A "/mnt/shared/logs/rclone-service-error.log" > /dev/null 2>&1
    exit 1
fi
# LOG PATH
...
#
RCLONE_SETTINGS=(
    "mount" "${RCLONE_MOUNT}" "${RCLONE_MOUNT_PATH}"
...
LOG_PATH="${LOG_PATH%/}/$(basename "$0" .sh)-${MOUNT_NAME}.log"
...
flock() { # Replacement for flock to fix trap issue
    ...
    if [[ -f $lock ]];  then
        ...
        if [[ -n $pid ]]; then
            active_pid=$(ps -e -o pid,cmd | awk -v a="$pid" -v b="${SCRIPT_PATH} ${MOUNT_NAME}" '$1==a && index($0,b) {print $1}')
    ...
    read -r pid < <(ps -e -o pid,cmd | awk -v a="${SCRIPT_PATH} ${MOUNT_NAME}" '$2!="awk" && index($0,a) {print $1}')
...
log() {
    ...
    while read -r INPUTLINE; do
        ...
        if [[ -n "${LOG_PATH}" ]]; then
            ...
            echo "no-file ${log_msg}" | tee -A "/mnt/shared/logs/rclone-service.log" > /dev/null 2>&1
        fi
    done
}
log_rotate() {
...
# lock script
flock "$(dirname -- "$SCRIPT_PATH")/lock_$(basename "$SCRIPT_PATH" .sh)_${MOUNT_NAME}"
